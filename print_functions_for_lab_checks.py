    ...
    if results_stats_dic is None:
        ...
        for key in results_dic:
            ...
            # NOT - match (not a breed match if a dog)
                ...
                # NOT - match
                # isa dog (pet label) 
                if len(results_dic[key]) > 4 and results_dic[key][3] == 1:
                    n_pet_dog += 1
        ...
        if n_pet_dog > 0: pct_corr_dog = ( n_class_cdog / n_pet_dog )*100
        if n_pet_notd > 0: pct_corr_notdog = ( n_class_cnotd / n_pet_notd )*100
        if n_pet_dog > 0: pct_corr_breed = ( n_match_breed / n_pet_dog )*100
